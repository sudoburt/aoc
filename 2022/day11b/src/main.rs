...
impl Monkey {
    pub fn from(lines: Vec<&str>) -> Monkey {
        ...
        let operation = match operation_line[0] {
            "+" => {
                match operation_line[1] {
                    ...
                    num => Operation::Add(num.parse::<u64>().unwrap()),
                }
            },
            "-" => Operation::Subtract(operation_line[1].parse::<u64>().unwrap()),
            "*" => {
                match operation_line[1] {
            ...
            "/" => Operation::Divide(operation_line[1].parse::<u64>().unwrap()),
            _ => unreachable!()
        };
        ...
        let test = match test_line[0] {
            "divisible" => Test::Divisible(test_line[1].parse::<u64>().unwrap()),
            _ => unreachable!()
        };
    ...
    pub fn take_turn(&mut self) -> Vec<(usize, u64)> {
        ...
        for item in &self.items {
            ...
            let new_worry_level = self.perform_operation(*item) % 9699690 as u64;
    ...
    fn perform_operation(&self, val: u64) -> u64 {
    ...
    fn perform_test(&self, val: u64) -> bool {
        match self.test {
